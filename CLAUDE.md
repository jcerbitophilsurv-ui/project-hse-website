...
Copy source of truth: `Branding/Horizon Solar - Website Build Sheet.docx` (43 numbered tasks, client-confirmed final copy) — see `PROGRESS.md` for what's been applied and what's deliberately deferred (the Privacy Policy page/consent copy pending legal sign-off). Pricing source of truth: `Pricing/Solar-Pricing-Reference-by-Consumption.xlsx`, transcribed into `content/pricing-tables.yml` — see "Content model" below and `PROGRESS.md` Phase 6.
...
quote.html                Instant solar savings calculator ("See What It Costs" destination site-wide) — one recommended system (Hybrid / Ongrid / Ongrid w/ Net Metering) looked up from content/pricing-tables.yml, not a formula
...
assets/js/quote-calculator.js Fetches quote-settings.yml + pricing-tables.yml, looks up the matching tier/product, renders one recommendation — only loaded on quote.html
